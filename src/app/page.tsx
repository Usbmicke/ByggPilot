        ...
        <header className="sticky top-0 z-50 bg-gray-900/80 backdrop-blur-sm">
            <div className="container mx-auto px-4 py-3 flex justify-between items-center">
                ...
                <nav className="flex items-center gap-4">
                    ...
                    <button onClick={() => toggleChat(true)} className="bg-cyan-400 text-black font-semibold px-4 py-2 rounded-md hover:bg-cyan-300 transition-colors">Testa ByggPilot Gratis</button>
                </nav>
            </div>
        </header>
    );
    ...
    return (
        <div className="h-screen w-full flex flex-col bg-gray-900">
            <header className="flex justify-between items-center p-4">
                <span className="font-bold text-xl text-white">ByggPilot</span>
                <button onClick={() => login()} className="bg-white text-gray-800 font-semibold py-2 px-4 rounded-lg flex items-center shadow-lg hover:bg-gray-200 transition-all">
                    <GoogleIcon /> Logga in för full åtkomst
                </button>
            </header>
            {/* Den globala Chat-komponenten fyller resten av ytan */}
        </div>
    );
};
...
const Page = () => {
    ...
    return (
        <>
            {isChatOpen ? <FullScreenChatView /> : <LandingPageContent />}
        </>
    );
};
